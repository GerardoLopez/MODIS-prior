...
def GetParameters(File, Bands, NumberOfParameters, RelativeUncert):
	"""
	Author
		Gerardo Lopez Saldana, ISA-UTL GerardoLopez@isa.utl.pt
	...
	Parameters = numpy.zeros((rows, cols, NumberOfBands, NumberOfParameters), numpy.uint16)
	...
	for Band in range(len(Bands)):
		for Parameter in range(NumberOfParameters):
			...
			Parameters[:,:,Band,Parameter] = numpy.where(Parameters[:,:,Band,Parameter]==FillValue, 0, 
                                                         Parameters[:,:,Band,Parameter])
...
# Extract, for the time being, only red, NIR & SWIR, bands 1, 2 & 7
...
for File in FileList:
	print File
	Parameters, Uncertainties = GetParameters(File, Bands, NumberOfParameters, RelativeUncert)
...
for i in range(NumberOfBands):
	for j in range(NumberOfParameters):
		...
		new_dataset.GetRasterBand((NumberOfBands*NumberOfParameters)+k).SetDescription("SD - band: " + str(Bands[i]) + " Parameter f" + str(j))
